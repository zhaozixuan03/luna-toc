...
- Completion and compression are independent. Long informative Prompts enter compression only when their normalized content exceeds 48 Unicode characters. The resolver selects one final semantic label; browser geometry only reports fit and cannot change meaning.
- The current productization increment is isolated on `codex/semantic-compression-boundary`; the audited conversation is development/regression material, not an unseen holdout.
...
- Smart Label cache schema is version 5. Version 4 is invalidated because it stored pre-layout semantic candidates. Missing signatures are unverifiable rather than implicit hits; consumed preceding Prompt and Assistant context participates in the signature. Capacity remains unchanged.
- Automated validation for the semantic-compression boundary: 20 focused tests pass, typecheck passes, build passes, evaluator passes all 4 short and 6 long reviewed fixtures, and `git diff --check` passes. The full 237-test run still exposes the pre-existing `renderedFingerprintCollector` timer instability (235 passed, 2 failed); an isolated replay passed 4 of 5.
- Chrome validation on the existing 83-Prompt conversation confirmed that Smart compression reaches the rendered sidebar, Raw restores the original Prompt, and resizing the sidebar from 300 px to 500 px and back leaves the selected Smart Label unchanged. The final punctuation cleanup is covered by an automated regression; the browser was not reloaded a second time because the user resumed control of Chrome.
...
- Merge the validated semantic-compression boundary increment, then handle three-line Raw/Smart rows and safer sidebar width as the next separate problem. Keep the intermittent empty-directory report open for observation rather than changing recovery logic without a stable reproduction.
...
- Reload `dist/` in Chrome and verify audited short prompts, reviewed long-prompt compression, Raw/Smart switching, search, hover provenance, regenerated responses, width-invariant semantic labels, and heading-only outline suppression. Cache version 5 invalidates older Smart Label records automatically.
